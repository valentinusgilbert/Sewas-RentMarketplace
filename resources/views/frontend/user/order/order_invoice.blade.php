    ...
    <table width="100%" style="border-collapse: collapse; padding-bottom: 10px;">
        <tr style="">
            <td align="right">
                ...
                <span style="opacity: 0.6;">{{ $order->invoice_no }}</span>
                {{-- <span style="opacity: 0.6;">{{ $order->invoice_no }}/00{{ $order->id }}/00{{ $order->user_id }}/{{ $order->order_number }}</span> --}}
            </td>
        </tr>
    </table>
    ...
    <table width="100%" style="padding: 10px 0 10px 0; line-height: 1.2;">
        ...
        <tbody>
            @foreach ($orderItem as $item)
                <tr style="font-size: 14px;">
                    <td>
                        <img src="{{ public_path($item->product->product_thambnail) }}" style="width: 85px">
                    </td>
